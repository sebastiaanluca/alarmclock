...
alarmJob = Schedule.scheduleJob({minute: null, second: [0, 20, 40]}, function () {
    debug('[ALARM1] Alarm triggered! It is now %s', Moment().format('MMMM Do YYYY, h:mm:ss a'));
    
    Schedule.scheduleJob('*/2 * * * * *', function () {
        debug('[ALARM 1 - SUB 2] Gradually increasing volume');
...
fixedSnoozeJob = Schedule.scheduleJob({minute: null, second: [10, 30, 50]}, function () {
    debug('[ALARM2] Force-snoozing alarm. Stopping playback. It is now %s', Moment().format('MMMM Do YYYY, h:mm:ss a'));
});
